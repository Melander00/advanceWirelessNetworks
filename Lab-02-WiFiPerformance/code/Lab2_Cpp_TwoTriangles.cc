// Lab 3: Infrastructure WiFi Scenario 3 (Four STAs + one AP in two equilateral triangles)
// Usage (from ns-3 root):
//   ./ns3 run "scratch/Lab3_Cpp_Scenario3 --rate=<Mbps> --seed=<run>"
...
// Map Mbps to valid 802.11b WifiMode string
...
int main (int argc, char *argv[])
{
  ...
  // Mobility: Two equilateral triangles around AP at (0,0)
  MobilityHelper mobility;
  Ptr<ListPositionAllocator> pos = CreateObject<ListPositionAllocator>();
  const double d = 10.0;

  // STA0, STA1, STA2 form first triangle with AP
  pos->Add(Vector(d, 0.0, 0.0));                            // STA0
  pos->Add(Vector(d/2, sqrt(d*d - (d/2)*(d/2)), 0.0));      // STA1

  pos->Add(Vector(-d, 0, 0.0));                             // STA2
  pos->Add(Vector(-d/2, -sqrt(d*d - (d/2)*(d/2)), 0.0));    // STA3
  ...
  Ipv4InterfaceContainer apIfaces  = address.Assign(apDevice);

  // Applications: two UDP OnOff clients (STA0→AP:9, STA3→AP:10)
  std::ostringstream offered;
  offered << rate << "Mbps";

  OnOffHelper onoff1("ns3::UdpSocketFactory",
                     InetSocketAddress(apIfaces.GetAddress(0), 9));
  ...
  OnOffHelper onoff2("ns3::UdpSocketFactory",
                     InetSocketAddress(apIfaces.GetAddress(0), 10));
  ...
  ApplicationContainer app2 = onoff2.Install(staNodes.Get(3));
  ...
  AnimationInterface anim("Lab2_TwoTriangles.xml");

  // Run simulation
  Simulator::Stop(Seconds(10.0));
  Simulator::Run();

  // Per-flow throughput via classifier
...
//   std::cout << "  aggregate throughput: " << thrAgg << " bps (" << thrAgg/1e6 << " Mbps)\n";
